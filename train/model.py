...
if __name__ == '__main__':
    ...
    # Model features
    num_feats = df.select_dtypes('number').drop(columns='Id').columns.drop('Notes')
    ...
    experience_transformer = Pipeline([
        ('Year', ExperienceTransformer('Year'))
    ])
    city_transformer = Pipeline([
        ('JobTitle', ce.cat_boost.CatBoostEncoder())
    ])
    transformers = ColumnTransformer([
        ('num_transformer', num_transformer, num_feats),
        ('Year', experience_transformer, ['Year']),
        ('JobTitle', city_transformer, ['JobTitle']),
    ])
    ...
    X_train, X_test, y_train, y_test = train_test_split(df.drop(columns=['Id']),
                                                        df['Id'],
                                                        test_size=0.2,
                                                        random_state=42)
